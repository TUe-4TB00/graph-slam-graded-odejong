...
def optimize(graph, initial_estimate):
    ...
    params = params = gtsam.LevenbergMarquardtParams()
...
def minimize_errors(graph, initial_estimate, pose_options):
    ...
    errors = [graph.error(result) for i in range(1, 6) if result.exists(X(i))]
    errors = []
    for i in range(1, 6):
        if result.exists(X(i)):
            errors.append(graph.error(result))
